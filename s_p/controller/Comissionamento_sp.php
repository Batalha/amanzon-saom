...
//        echo die_json($_SESSION);
	...
	public function inicia_comiss()
    {
    	...
    	$data = array(
    		...
    		'resp_comiss_time' => $dataAtual,
    		'create_user_comiss' => $_SESSION['login']['idusuarios'],
    		'resp_user_comiss' => $_SESSION['login']['idusuarios']
    	);
    	...
    	if ( ! empty($this->dadosP['param']) )
       	{
			...
			$dados['responsavel_comiss'] = $this->Usuarios_sp->fetchRow(" idusuarios = '{$dados['resp_user_comiss']}' ");
			$dados['comissionado_por'] = ($dados['comissionado_por'] instanceof Zend_Db_Table_Row )?$dados['comissionado_por']->nome:'';
			$dados['responsavel_comiss'] = ($dados['responsavel_comiss'] instanceof Zend_Db_Table_Row )?$dados['responsavel_comiss']->nome:'';
			...
			$dados['resp_comiss_time'] = ( $dados['resp_comiss_time'] != '' )?$this->Helpers->data_us_br_com_hora($dados['resp_comiss_time']):'';
...
// 		    $this->alterandoCaminhoDoTermoAceiteNoBanco();
	...
	public function despausar(){
		...
		{
			$arrReturn['status'] = 'ok';
			$arrReturn['msg'] = 'Comiss despausado com sucesso.';
		}
		else
		{
...
//		$arrReturn['msg']     = $dataAtual;
//		die_json($arrReturn);
		$sql = "
		UPDATE instalacoes_sp
		SET data_pausa = '{$dataAtual}'
		WHERE idinstalacoes_sp = '{$this->dadosP['form']['idinstalacoes_sp']}';	";

		if(	$this->DB->query($sql))
		{
			$arrReturn['status'] = 'ok';
			$arrReturn['msg'] = 'Comiss pausada com sucesso.';
		}
		else
		{
			$arrReturn['status'] = 'erro';
...
//         		echo die_json($dados);
        ...
        elseif ( ! empty($this->dadosP['form']))//SUBMIT
        {
			...
			if( $this->dadosP['form']['mac_comiss'] != '' )
				$this->Equipamento_sp->verifica_existencia_mac( $this->dadosP['form']['mac_comiss'] );
...
//				<img src='http://saom.globaleagle.com.br/public/imagens/logoEMC.png' height='100' width='300'/>
//			";
//
...
//    		else //EMAIL PARA PRODEMGE - fim
    ...
    public function resgataODUdeNSODU()
    {
    	...
    	if( count($odu) > 0 ) {
			...
			echo $odu[0]['tipo_equipamentos_sp_idtipo_equipamentos_sp'];
		}else
			echo '';
    }
}
