...
require_once 's_p/controller/MailManager_sp.php';
require_once 's_p/controller/PreIncidentes_sp.php';

class MailIncidenteProdemge_sp extends MailManager_sp
{
	...
	private function inserePreIncidentes( Array $listaPreIncidentes )
	{
		...
		{
			$preIncidenteObject = new PreIncidentes_sp();
			...
			if( $this->PreIncidentes_sp->insert( $data ) ){
				$retorno = true;
			}else{
				$retorno = false;
			}
	...
	private function verificaPreExistenciaPreIncidente( PreIncidentes $preIncidente )
	{
		...
		$preIncidentesEncontrados =  $this->PreIncidentes_sp->fetchAll( $where );
