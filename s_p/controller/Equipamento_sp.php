...
require_once 's_p/model/Equipamentos_spModel.php';
require_once 's_p/model/EquipamentosAntenas_spModel.php';
require_once 's_p/model/EquipamentosLocais_spModel.php';
require_once 's_p/model/LocaisEquipamentos_spModel.php';
require_once 's_p/model/TipoEquipamentos_spModel.php';
require_once 's_p/model/Municipios_spModel.php';
require_once 's_p/model/Instalacoes_spModel.php';
require_once 's_p/model/EquipamentosAntenasLocais_spModel.php';

include_once 's_p/model/DBEquipamento_sp.php';
include_once 's_p/model/DBTipoEquipamento_sp.php';

include_once '/libs/phpexcel/PHPExcel/IOFactory.php';

require_once 'helpers/Controller.php';



class Equipamento_sp extends Controller 
{    
...
//    protected $listaLocaisEquipamentos;
    ...
    protected $tplDir = 's_p/tampletes/equipamento';
    
    function __construct() 
    {

        parent::__construct();
        $this->DB = new DBEquipamento_sp();
    }
...
//		   echo die_json($this->listaTipoEquipamentos->toArray());
...
//            $this->smarty->assign('listaLocaisEquipamentos',$this->listaLocaisEquipamentos->toArray());
            $this->smarty->display("{$this->tplDir}/creat.tpl");
       }
       else 
       {
//            $this->trataLocal();
            ...
            $equipamentos = new Equipamentos_spModel( $this->adapter->getAdapterZend() );
...
//		   echo die_json($this->dadosP['form']);
		   ...
		   $liste = $this->DB->liste();
		   foreach($liste as $chave=>$equip){


			   if($this->dadosP['form']['sno'] == $equip['sno']){
				   if($equip['mac']){
				   		$arrReturn = array('status' => 'erro','msg' => 'Esse MAC Já exite.');
				   }else{
				   		$arrReturn = array('status' => 'erro','msg' => 'Essa ODU Ja Existe.');
    ...
    public function view()
    {
    	...
    	$equipamento = new Equipamentos_spModel( $this->adapter->getAdapterZend() );
		$equipamento_linha = $equipamento->fetchRow("idequipamentos_sp = '{$this->id}'");
...
//            var_dump('testess');
        	...
        	$retorno = $equipamento->update( $this->dadosP['form'] , "idequipamentos_sp = '{$this->id}'" );
...
//		echo die_json($where);
		...
		$where = "idequipamentos_sp = {$this->id}";

		if($equipamentos->delete($where))
    		return "ok";
		else
			return false;
    }
...
//		print_b($data,true);
    ...
    {
    	$idequipamentos = $_REQUEST['idequipamentos_sp'];
    	
    	$this->DB->setSelect(' observacoes ');
    	$dados = $this->DB->liste(" idequipamentos_sp = '{$idequipamentos}' ");
    	//print_b($dados,true);
    ...
    {
    	$sql = "SELECT e.status FROM equipamentos_sp e WHERE e.sno = '{$nsoduNovo}' ";
    ...
    public function buscaNSVsatPorMac()
    {
    	...
    	echo (isset($dados[0]['sno']))?$dados[0]['sno']:'';

    }

	public function buscaNSAgendaPorMac()
	{
		$mac = $_POST['mac'];
    	...
    	";
			...
			if($dados[0]['sno']!='' && $dados[0]['status'] == 1){
//				echo 'mac';
				echo $dados[0]['sno'];
			}else if($dados[0]['sno']!=''&& $dados[0]['status'] == 2){
					echo 'emUso';
			}else{
				echo 'macVazio';
			}
    ...
    {
    	$tipoEquipamentos = new TipoEquipamentos_spModel( $this->adapter->getAdapterZend() );
    	$this->listaTipoEquipamentos = $tipoEquipamentos->fetchAll();

    	//locais ---
    	
//    	$municipios = new Municipios_spModel( $this->adapter->getAdapterZend() );
//    	$this->listaMunicipios = $municipios->fetchAll('1','municipio ASC');

    	$instalacoes = new Instalacoes_spModel( $this->adapter->getAdapterZend() );
    	//TODO: metodo vai para model
    	$where = "
            	(
            		SELECT COUNT(*) 
            		FROM equipamentos_locais_sp
            		WHERE 
            			idlocais_equipamentos = instalacoes_sp.idinstalacoes_sp AND
            			tabela_localidade = 'locais_equipametos' 
            	) = 0
            ";//apenas desocupadas
    	$this->listaInstalacoes = $instalacoes->fetchAll($where,'nome ASC');

    	$locaisEquipamentos = new LocaisEquipamentos_spModel( $this->adapter->getAdapterZend() );
    ...
    {
    	$equipamento = new Equipamentos_spModel( $this->adapter->getAdapterZend() );
    	$linha = $equipamento->fetchRow("idequipamentos_sp = '{$this->id}'");
    ...
    public function carregaDependencias()
    {
    	...
    	$equipamentoDependencias = new EquipamentosLocais_spModel( $this->adapter->getAdapterZend() );
	   	$associacoes = $equipamentoDependencias->fetchRow("idequipamentos_sp = '{$this->id}'");
...
//
	   			case 'instalacoes_sp':
	   				$instalacoes = new Instalacoes_spModel( $this->adapter->getAdapterZend() );
	   				$local = $instalacoes->fetchRow("idinstalacoes_sp = '{$associacoes->idlocais_equipamentos}'");

	   				break;
	   			case 'locais_equipamentos_sp':
	   				$locais_equipamentos = new LocaisEquipamentos_spModel( $this->adapter->getAdapterZend() );
	   	...
	   	$tipo_equipamento = new TipoEquipamentos_spModel( $this->adapter->getAdapterZend() );
	   	$tipo_equipamento_linha = $tipo_equipamento->fetchRow("idtipo_equipamentos_sp = '{$this->equipamento['tipo_equipamentos_sp_idtipo_equipamentos_sp']}'");
    ...
    public function gravaLocal()
    {
    	...
    	if ($localTabela == 'instalacoes_sp'){
	    	$sql = "SELECT idinstalacoes_sp FROM instalacoes_sp
	    			WHERE idinstalacoes_sp = $localId
	    			";
	    	$dadosTable = $this->DB->queryDados($sql);
	    	$localId = $dadosTable[0]['idinstalacoes_sp'];
    	}else if ($localTabela == 'municipios_sp' ){
	    	$sql = "SELECT i.idinstalacoes_sp, i.nome
	    			FROM instalacoes_sp i, os_sp o
	    			WHERE o.idos = i.os_sp_idos and o.municipios_sp_idcidade = $localId
	    			";
    	...
    	$dados = array(
    		'idequipamentos_sp' => $this->id,
    		'idlocais_equipamentos' => $localId,
    		'tabela_localidade' => $localTabela
    	);
    	
   	
    	$equipamentosLocais = new EquipamentosLocais_spModel( $this->adapter->getAdapterZend() );

    	if (!$this->verificaStatus()){ //
	    	if ($localId > 0){
				if (!$equipamentosLocais->update( $dados, "idequipamentos_sp = '{$this->id}'" )){
					$sql = "SELECT idequipamentos_sp FROM equipamentos_locais_sp
							WHERE idequipamentos_sp = '{$this->id}'";
    ...
    public function verificaStatus(){
    	...
    	if ($this->status == 1 || $this->status == 2 || $this->status == 3 ){
    		$equipamentosLocais->delete("idequipamentos_sp = '{$this->id}'");
	    	return true;    	
    	}
    }
    ...
    public function gravaLocalCreateMulti( $idEquipamento )
    {
    	//TODO: vai para model
    	$sql = "
    		INSERT INTO equipamentos_locais_sp ( idequipamentos_sp , idlocais_equipamentos , tabela_localidade )
    ...
    {
    	$this->id = $this->dadosP['form']['idequipamentos_sp'];
    	unset($this->dadosP['form']['idequipamentos_sp']);
    ...
    public function atualizaDependencias()
    {
    	...
    	$equipamentoLocal = $equipamentosLocais->fetchRow("idequipamentos_sp = '{$this->id}'");
    	
    	$dados = array(
		    "idequipamentos_sp" => $equipamentoLocal->idequipamentos_sp,
    ...
    public function gravaBD()
    {
    	...
    	foreach( $this->dadosPlanilha AS $dado )
    	{
	    	...
	    	//grava equipamento
	    	$sql = "
			   	INSERT INTO equipamentos_sp
			   	( sno , mac , tipo_equipamentos_sp_idtipo_equipamentos_sp , observacoes )
			   	VALUES
    ...
    public function resolveTipoLocal($dado)
    {
    	...
    	{
    		case 'municipio':
	    		$this->tipoLocal = 'municipios_sp';
	    		break;
	    	case 'vsat':
	    		$this->tipoLocal = 'instalacoes_sp';
	    		break;
	    	case 'local_equipamento':
	    		$this->tipoLocal = 'locais_equipamentos_sp';
	    		break;
    	} 
    }
    ...
    public function resgataTotalAntenas( $tipo_equipamento_antenas = '' )
    {
    	...
    	$equipamentos_antenas = new EquipamentosAntenas_spModel( $this->adapter->getAdapterZend() );
    ...
    public function resgateAntenasOcupadas( $tipo_equipamento_antenas = '' )
    {
    	...
    	$equipamentos_antenas_locais = new EquipamentosAntenasLocais_spModel( $this->adapter->getAdapterZend() );
