...
include_once 's_p/model/DBRelatorio_sp.php';

class Relatorio_sp extends Controller
{
	
	protected $tplDir = 's_p/tampletes/relatorio';
	...
	public function novaListaAtivacaoVsatsDia()
	{
		...
		$sql = "
			SELECT i.nome AS nome,i.data_ativacao AS data_ativacao,(
		        	SELECT data	FROM agenda_instal_sp WHERE os_sp_idos = i.os_sp_idos) AS data_agendamento,
		           i.data_aceite AS aceite_prodemge
		    FROM instalacoes i
...
// 		echo die_json($lista[1]['nome']);
	...
	private function buscaInstalacoes( Array $listaInstalacoes )
	{
		...
		{
			$where = "
				idinstalacoes_sp = '{$instalacao}'
			";
			$listaInstalacoes[ $chave ] = $this->Instalacao_sp->fetchAll( $where );
		}
	...
	private function buscaIncidentesDeCadaInstalacao( Array $listaInstalacoesRowSet )
	{
		...
		foreach ( $listaInstalacoesRowSet as $chave => $instalacao )
		{
			...
			{
				$where = "
					instalacoes_idinstalacoes = '{$instalacao[0]->idinstalacoes_sp}'
				";
