...
// 		print_r($this->array_acompanhamento_noc);
	...
	public function acompanhamentoNoc()
	{
		...
		$tecnico 			= $_SESSION['dataTecnico'];
		$tipo 				= $_SESSION['dataTipo'];
		$data_inicio	= $_SESSION['dataData'];
		$solicitacao	= $_SESSION['dataSolicitacao'];
		$incidente	 	= $_SESSION['dataIncidente'];
		$vsat 				= $_SESSION['dataVsat'];
		$status 			= $_SESSION['dataStatus'];
		$data_fim 		= $_SESSION['dataDataFin'];

		$this->smarty->assign('tecnico',$tecnico);
		$this->smarty->assign('tipo',$tipo);
		$this->smarty->assign('data_inicio',$data_inicio);
		$this->smarty->assign('solicitacao',$solicitacao);
		$this->smarty->assign('incidente',$incidente);
		$this->smarty->assign('vsat',$vsat);
		$this->smarty->assign('status',$status);
		$this->smarty->assign('data_fim',$data_fim);
		$this->smarty->display("{$this->tplDir}/listchamadosfull.tpl");
	}
        ...
        public function acompanhamentoNocConteudo()
	{
		...
		$filtroCampoTecnico = $_SESSION['campoTecnico'];
		$filtroNomeTecnico  = $_SESSION['dataTecnico'];
		$filtroCampoSolicitacao = $_SESSION['campoSolicitacao'];
		$filtroNomeSolicitacao  = $_SESSION['dataSolicitacao'];
		$filtroCampoVsat = $_SESSION['campoVsat'];
		$filtroNomeVsat  = $_SESSION['dataVsat'];
		$filtroCampoIncidente = $_SESSION['campoIncidente'];
		$filtroNomeIncidente  = $_SESSION['dataIncidente'];
		$filtroCampoStatus = $_SESSION['campoStatus'];
		$filtroNomeStatus  = $_SESSION['dataStatus'];
...
//			// ---------------- FILTRA PERFIS DA SESSAO
			...
			elseif(
				(stristr($this->array_acompanhamento_noc[$chave]['cell'][$filtroCampoTecnico], $filtroNomeTecnico) != $this->array_acompanhamento_noc[$chave]['cell'][$filtroCampoTecnico])||
				(stristr($this->array_acompanhamento_noc[$chave]['cell'][$filtroCampoSolicitacao], $filtroNomeSolicitacao) != $this->array_acompanhamento_noc[$chave]['cell'][$filtroCampoSolicitacao])||
				(stristr($this->array_acompanhamento_noc[$chave]['cell'][$filtroCampoVsat], $filtroNomeVsat) != $this->array_acompanhamento_noc[$chave]['cell'][$filtroCampoVsat])||
				(stristr($this->array_acompanhamento_noc[$chave]['cell'][$filtroCampoIncidente], $filtroNomeIncidente) != $this->array_acompanhamento_noc[$chave]['cell'][$filtroCampoIncidente])||
				(stristr($this->array_acompanhamento_noc[$chave]['cell'][$filtroCampoStatus], $filtroNomeStatus) != $this->array_acompanhamento_noc[$chave]['cell'][$filtroCampoStatus])||
				(
					$this->array_acompanhamento_noc[$chave]['cell']['sala'] != 2
				)
			){
...
//
        ...
        $arrayDados['total'] = (count( $this->array_acompanhamento_noc ))?count( $this->array_acompanhamento_noc ):1;
		$arrayDados['page'] = $this->page;
		$arrayDados['rows'] = $this->array_acompanhamento_noc;
	...
	public function acompanhamentoNocConteudoFiltro()
	{
		$contagemAtual = 0;
		...
		if($this->query != ''){
			unset(
				$_SESSION['campoTecnico'], $_SESSION['dataTecnico'],$_SESSION['campoSolicitacao'],$_SESSION['dataSolicitacao'],
				$_SESSION['campoVsat'],$_SESSION['dataVsat'],$_SESSION['campoIncidente'],$_SESSION['dataIncidente'],
				$_SESSION['campoStatus'],$_SESSION['dataStatus']
			);
			$separarAnd = explode("AND", preg_replace("/[^a-zA-Z0-9\s]/", "", $_POST['query']));


			for($and = 0;  $and<=count($separarAnd); $and++){
				$separarLike = explode("LIKE", $separarAnd[$and]);
				$separarLike = preg_replace("/[^a-zA-Z0-9]/", "", $separarLike);
				if($separarLike[0] == 'tecnico'){
					$_SESSION['campoTecnico'] = $separarLike[0];
					$_SESSION['dataTecnico'] = $separarLike[1];
				}elseif($separarLike[0] == 'solicitacao'){
					$_SESSION['campoSolicitacao'] = $separarLike[0];
					$_SESSION['dataSolicitacao'] = $separarLike[1];
				}elseif($separarLike[0] == 'vsat'){
					$_SESSION['campoVsat'] = $separarLike[0];
					$_SESSION['dataVsat'] = $separarLike[1];
				}elseif($separarLike[0] == 'incidente'){
					$_SESSION['campoIncidente'] = $separarLike[0];
					$_SESSION['dataIncidente'] = $separarLike[1];
				}elseif($separarLike[0] == 'status'){
					$_SESSION['campoStatus'] = $separarLike[0];
					$_SESSION['dataStatus'] = $separarLike[1];
				}
			}
		}else{
			unset(
				$_SESSION['campoTecnico'],$_SESSION['dataTecnico'],
				$_SESSION['campoSolicitacao'],$_SESSION['dataSolicitacao'],
				$_SESSION['campoVsat'],$_SESSION['dataVsat'],
				$_SESSION['campoIncidente'],$_SESSION['dataIncidente'],
				$_SESSION['campoStatus'],$_SESSION['dataStatus']
			);
		}
		$filtroCampoTecnico = $_SESSION['campoTecnico'];
...
//			// ---------------- FILTRA PERFIS DA SESSAO
			...
			if (
				...
				$this->array_acompanhamento_noc[$chave]['cell']['tempo_vencimento'] = $intervalo;
			} elseif ($acompanhamento['cell']['idperfil_usuario'] != 1 && $acompanhamento['cell']['idperfil_usuario'] != 4){
				unset($this->array_acompanhamento_noc[$chave]);// retira usuarios que 'não tem perfil NOC'
			}
...
//			elseif(stristr($this->array_acompanhamento_noc[$chave]['cell']['solicitacao'], 'Do') != $this->array_acompanhamento_noc[$chave]['cell']['solicitacao']){
//
//				unset($this->array_acompanhamento_noc[$chave]);
//			}
...
//			$this->array_acompanhamento_noc[ $chave ]['cell']['hub'] = '';
			...
			if( 
				...
				(
					$this->array_acompanhamento_noc[$chave]['cell']['sala'] != 3
				)
			){
				unset( $this->array_acompanhamento_noc[ $chave ] );
			}else{
...
//		$a['msg'] = $atendimentosAbertos[0]['sala'];
//		die_json($a);
				...
				foreach($osspId as $chave=>$osspIds){
					...
					if($install = $this->Instalacao_sp->fetchRow($surch)){
						if($install['idinstalacoes_sp'] == $atendimento['instalacoes_sp_idinstalacoes_sp']){
							...
							if($atendimento['solicitacao_sp_idsolicitacao']){

								$solicitacao = $this->Solicitacao_sp
									->find($atendimento['solicitacao_sp_idsolicitacao'])
									->toArray();
								$atendimento['solicitacao']		= $solicitacao[0]['nomeSolicitacao'];
							...
							$atendimento = $this->listaAcompanhamentoNoc($atendimento);
			...
			foreach ($atendimentosAbertos as $atendimento) {
				...
				if ($atendimento['usuarios_idusuarios']) {
					$usuario = $this->Usuarios_sp
						->find($atendimento['usuarios_idusuarios'])
						->toArray();
					$atendimento['tecnico']        = $usuario[0]['nome'];
					$atendimento['perfil_usuario'] = $usuario[0]['perfis_idperfis'];
					$atendimento['perfil_atual'] = $_SESSION['login']['perfis_idperfis'];
	...
	public function listaAcompanhamentoNoc($atendimento){
		...
		return $atendimento = array(
			...
			'cell' => array(
				...
				'pausa'            => $atendimento['data_pausa'],
				'idperfil_usuario' => $atendimento['perfil_usuario'],
				'perfil' 		   => $atendimento['ordem'],
...
//---------------------------------------DADOS COMISSIONAMENTOS----------------------------------------------------------
	...
	public function busca_comissionamentos()
	{
		...
		if( $instalacoes instanceof Zend_Db_Table_Rowset)
		{
			...
			{
				$where = "idusuarios = '{$instalacao['resp_user_comiss']}'";
