...
class TermoResponsabilidade_sp extends Controller implements TermoResponsabilidadeInterface
{
	protected $tplDir = 's_p/tampletes/termoresponsabilidade';
	protected $pastaDeTermoResponsabilidade = 'upload/termo_responsabilidade_sp/';
	...
	public function upload()
	{
    	...
    	if( $this->enviaServidor( $this->dadosF , $this->pastaDeTermoResponsabilidade ) )
    	{
    		...
    		$arquivo = "<a id='arquivo_novo_termo_responsabilidade' target='_blank' href='upload/termo_responsabilidade_sp/{$this->dadosF['termo_responsabilidade']['name']}'><i class='icon-file'></i><font style='color:#000;'>{$this->dadosF['termo_responsabilidade']['name']}</font></a>";
	...
	{    
		...
		$this->TermoResponsabilidade_sp->getTermoResponsabilidade( new Integer($id_instalacoes) ); // busca termo pre existente
		
		if( $this->TermoResponsabilidade_sp->termo != false ) // havendo pre exist̻ncia
		{
			if( $this->TermoResponsabilidade_sp->updateTermoResponsabilidade( $dados , new Integer($this->TermoResponsabilidade_sp->termo['id_termo_responsabilidade']) ) == false )
		...
		else // novo termo de responsabilidade
		{
			...
			if( $this->TermoResponsabilidade_sp->insertTermoResponsabilidade( $dados ) == false )
	...
	{
		if( $this->TermoResponsabilidade_sp->aprova( new Integer($this->dadosP['idTermo']) ) === true )
	...
	{
		$this->TermoResponsabilidade_sp->getTermoResponsabilidade( new Integer($this->dadosP['idTermo']) );
		if( 
			$this->TermoResponsabilidade_sp->desaprova( new Integer($this->dadosP['idTermo']) ) === true
			|| $this->TermoResponsabilidade_sp->termo['status'] == 2
		)
		{
			$this->TermoResponsabilidade_sp->getTermoResponsabilidade( new Integer($this->dadosP['idTermo']) );
			
			$this->smarty->assign( 'termo' , $this->TermoResponsabilidade_sp->termo );
	...
	{
		if( $this->TermoResponsabilidade_sp->justificaDesaprovacao( new Integer($this->dadosP['id_termo_responsabilidade']) , $this->dadosP['comentario'] ) )
	...
	public function apagarTermoDeResponsabilidade()
	{
		...
		$this->TermoResponsabilidade_sp->getTermoResponsabilidade( new Integer($id_termo_responsabilidade) );
		
		$arquivo = $this->TermoResponsabilidade_sp->termo['endereco'].$this->TermoResponsabilidade_sp->termo['nome'];
		unset($arquivo);
		
		if( $this->TermoResponsabilidade_sp->deleteTermoResponsabilidade( new Integer($this->TermoResponsabilidade_sp->termo['id_termo_responsabilidade']) ) )
	...
	public function apagarTermoDeResponsabilidadePeloIdInstalacoes()
	{
		...
		if( $this->TermoResponsabilidade_sp->deleteTermoResponsabilidadePeloIdInstalacao( new Integer($id_instalacoes) ) )
			die("<span class='alert alert-success'>Termo de Responsabilidade apagado com sucesso.</span>");
		else
			die("<span class='alert alert-error'>Erro ao apagar Termo de Responsabilidade.</span>");
	}
