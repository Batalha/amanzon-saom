...
<table class="tbLista" id="OSlist">
    <tr>
        ...
        <th>Macroregião</th>
        <th>Data Solic.</th>
        <th class='titleTable'><a href="#" onclick="javascript:getAjaxForm('OSSP/liste','conteudo',{ldelim}orderBy:'prazoInstal',ajax:1{rdelim})"  >Prazo</a></th>
        <th>Agendamento</th>
        <th>Vsat criada?</th>
        <th>Comiss.</th>
        <th>Cod. Anatel</th>
    ...
    <tr class="{$rowCss}" onClick="javascript:getAjaxForm('OSSP/view','conteudo',{ldelim}param:{$os.idos},ajax:1{rdelim})" onMouseOver="javascript:onOver(this)" onMouseOut="javascript:onUp(this,'{$rowCss}')">
        
        <td>{$os.numOS}</td>
        
        <td>SES</td>
        <td>{$os.rel.municipios_sp_idcidade.municipio}</td>
        <td>{$os.rel.municipios_sp_idcidade.macroregiao}</td>
