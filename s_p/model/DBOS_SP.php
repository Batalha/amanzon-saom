...
class DBOS_SP extends DBModel_sp {
	...
	protected $rel = array(
		'instalacoes_sp','agenda_instal_sp','empresas','satelite','escricao_fornecimento', 'os_sp_servicos'
	);//'municipios_sp', ,'satelite','escricao_fornecimento'
	protected $fgk = array('idos','idos','empresas_idempresas'/*,'empreiteira_idempresas'*/,'satelite_idsatelite','escricao_fornecimento_idescricao_fornecimento','idos');//,'idos' ,'satelite_idsatelite','escricao_fornecimento_idescricao_fornecimento'
...
//        	$this->query($sql2);
	...
	public function lista_os_sp_servicos($id)
	{
		$sql = "SELECT * FROM os_sp_servicos WHERE os_sp_idos = '{$id}' ORDER BY id DESC ";
		return $this->queryDados($sql);

	}

	public function view_os_atual($id)
	{
//		$sql = "SELECT id, os_sp_idos, num_os_sp, status_atual, data, observacoes FROM os_sp_servicos WHERE id = {$idos} ORDER BY id ASC";
		$sql = "SELECT * FROM os_sp_servicos WHERE id = {$id}";
		$return = $this->queryDados($sql);
