...
class DBInstalacao_sp extends DBModel_sp {
    ...
    protected $tabela = 'instalacoes_sp';
    protected $cmpData= array(
    						'data_aceite',
    						'data_ativacao'
    						);
    
    protected $prk    = 'idinstalacoes_sp';
...
//    							'cabo_rj45'
    ...
    //SETS
    	...
    	{
    		$this->idinstalacoes_sp = $idinstalacoesnovo;
    	}
    //SETS - fim
    ...
    public function getName($name)
    {
       ...
       $sql      = "SELECT COUNT(*) as total FROM instalacoes_sp WHERE nome LIKE '".$name."'";
       $nameVsat = $name;
       ...
       if($ifun){
         ...
         while($ifun ){
           ...
           $sql = "SELECT COUNT(*) as total FROM instalacoes_sp WHERE nome LIKE '".$nameVsat."'";
           $ifun = $this->queryDados($sql);
           $ifun = $ifun[0]['total'];
           $i++;
    ...
    public function countPendShapper()
    {
    	if($_SESSION['login']['empresas_idempresas']==1)//VODANET
     	{
        	$sql = "SELECT COUNT(i.idinstalacoes_sp) AS total FROM instalacoes_sp i WHERE ISNULL(i.packetshapper)";
        }
     	else
     	{
     		$sql = "SELECT COUNT(i.idinstalacoes_sp) AS total FROM instalacoes_sp i WHERE
     			(SELECT o.empresas_idempresas FROM os_sp o WHERE o.idos = i.idinstalacoes_sp) = {$_SESSION['login']['empresas_idempresas']} AND
    ...
    public function countPendWNMS()
    {
    	if($_SESSION['login']['empresas_idempresas']==1)//VODANET
     	{
        	$sql = "
        		SELECT COUNT(i.idinstalacoes_sp) AS total
     		...
     		$sql = "
     			...
     			WHERE
     				(
     					...
     					FROM os_sp o 
     					WHERE o.idos = i.idinstalacoes_sp
    ...
    public function countComiss()
    {
    	if($_SESSION['login']['empresas_idempresas']==1)//VODANET
     	{
    		$sql = "SELECT COUNT(i.idinstalacoes_sp) AS total FROM instalacoes_sp i WHERE ISNULL(i.comiss) ";
     	}
     	else
     	{
     		$sql = "
    ...
    {
    	$sql = " SELECT te.idtipo_equipamentos, te.nome FROM tipo_equipamentos_sp te WHERE idtipo_equipamentos = (SELECT e.tipo_equipamentos_sp_idtipo_equipamentos FROM equipamentos_sp e WHERE sno = '{$nsodu}') ";
