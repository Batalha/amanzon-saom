...
class DBMonitor_sp extends DBModel_sp {
    ...
    public function init(){
        ...
        foreach($arr as $i) {
            ...
            $sqlCount = "SELECT count(*) as total FROM monitor_sp WHERE USERNAME LIKE '".$i."' AND LOGGED_ON LIKE 'N'";
            $dadosCount = $this->queryDados($sqlCount);
           
            if($dadosCount[0]['total']){
                
                $sqlAtual = "SELECT * FROM monitor_sp WHERE USERNAME LIKE '".$i."' ORDER BY ID DESC LIMIT 1";
                ...
                $sqlOff = "SELECT * FROM monitor_sp WHERE USERNAME LIKE '".$i."' AND LOGGED_ON LIKE 'N'";
