...
class Equipamentos_spBO extends Equipamentos_spModel
{
	protected $nomes_equipamento = array(
		'nsmodem_comiss' => 'Número de Série do Modem',
		'nsodu_comiss' => 'Número de Série do ODU/BUC'
	);
	...
	public function valida_ns(
								...
								$equipamento ,
								EquipamentosLocais_spBO $equipamentosLocais
								)
	{
		$where = "sno = '{$ns}'";
														...
														)
	{
		...
		if( count($local_equipamento) > 0 )
		{
			...
			if( $local_equipamento[0]->idlocais_equipamentos != $instalacao->idinstalacoes_sp )
			{
												...
												)
	{
		// ---------- atualiza status do equipamento anterior
			$where = "
				tabela_localidade = 'instalacoes_sp' AND
				idlocais_equipamentos = '{$instalacao->idinstalacoes_sp}' AND
				idequipamentos_sp != '{$ns_carregado[0]->idequipamentos_sp}' AND
...
//		die_json($arrReturn);
		...
		// ---------- atualiza status do equipamento novo
			...
			if( count($equipamento_carregado) > 0 )
			{
				...
				$where = " idequipamentos_sp = '{$equipamento_carregado[0]->idequipamentos_sp}' ";
				$equipamento_locais = $equipamentosLocais->fetchAll( $where );
				if( count($equipamento_locais) > 0 )
					$this->atualiza_dados_local_equipamento( $instalacao->idinstalacoes_sp , $equipamento , $equipamento_locais , $equipamentosLocais );

				else
					$this->insere_dados_local_equipamento( $equipamento_carregado[0]->idequipamentos_sp , $instalacao->idinstalacoes_sp , $equipamento , $equipamentosLocais );
			}
	}
...
//        $arrReturn['msg'] = $associacao_equipamento[0]->idequipamentos_sp;
//        die_json($arrReturn);
		$where = " idequipamentos_sp = '{$associacao_equipamento[0]->idequipamentos_sp}' ";
		...
		{
			$arrReturn['status'] = 'erro';
	    	$arrReturn['msg']    = "Erro ao mudar status do {$this->nomes_equipamento[$equipamento]}. Erro na linha 154 de equipamentos_spBo.";
	    	die_json($arrReturn);
		}
	}
	...
	public function atualiza_dados_local_equipamento( $idinstalacoes , $equipamento , Zend_Db_Table_Rowset $equipamento_local , EquipamentosLocais_spBO $equipamentos_locais )
	{
		if( 
			$equipamento_local[0]->tabela_localidade != 'instalacoes_sp' ||
			( $equipamento_local[0]->tabela_localidade == 'instalacoes_sp' && $equipamento_local[0]->idlocais_equipamentos != $idinstalacoes )
		)
		{
			$data = array(
				'tabela_localidade' => 'instalacoes_sp',
	...
	public function insere_dados_local_equipamento( $idequipamentos , $idinstalacoes , $equipamento , EquipamentosLocais_spBO $equipamentos_locais )
	{
		$data = array(
			'idequipamentos_sp' => $idequipamentos,
			'idlocais_equipamentos' => $idinstalacoes,
			'tabela_localidade' => 'instalacoes_sp',
			'tipo_associacao' => $equipamento
		);
	...
	{
		$where = " idequipamentos_sp = '{$ns_carregado[0]->idequipamentos_sp}' AND tabela_localidade = 'instalacoes_sp' AND tipo_associacao = '{$equipamento}' ";
