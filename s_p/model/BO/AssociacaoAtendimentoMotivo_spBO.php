...
require_once "s_p/model/AssociacaoAtendimentoMotivo_spModel.php";

class AssociacaoAtendimentoMotivo_spBO extends AssociacaoAtendimentoMotivo_spModel
{
...
// 		";
		...
		FROM atendimento_motivo_responsavel_sp a
		LEFT JOIN motivo_atendimento_sp m ON m.idmotivo_atendimento = a.idmotivo
		LEFT JOIN responsavel_atendimento_sp ra ON ra.idresponsavel_atendimento = a.idresponsavel
		WHERE a.idatendimento = '{$idatendimento}';
		";
...
// 			{
				$sql = "
					UPDATE atendimento_motivo_responsavel_sp 
			...
			foreach ( $incidente['atendimentos'] as $chaveAtendimento => $atendimento )
			{
				$sql = "
					SELECT 
						a.idmotivo as idmotivo,
						m.motivo as motivo
					FROM associacao_atendimento_motivo_sp a
					LEFT JOIN motivo_atendimento_sp m ON m.idmotivo_atendimento = a.idmotivo
