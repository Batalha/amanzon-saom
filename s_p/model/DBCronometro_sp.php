...
class DBCronometro_sp extends DBModel_sp
{
	...
	//padrao
	protected $tabela = 'cronometro_sp';
    protected $rel    = array();
    protected $fgk    = array();
    protected $prk    = 'idcronometro_sp';//primary key
    ...
    {
    	$sql = "SELECT idcronometro, idreferencia, inicio_tarefa, tabelareferencia, final_tarefa, prazo FROM cronometro_sp WHERE idcronometro = {$this->idcronometro}";
    ...
    {
    	$sql = "SELECT * FROM cronometro_sp WHERE tabelareferencia = '{$this->tabelareferencia}' AND idreferencia = '{$this->idreferencia}' ";
    	$dados = $this->queryDados($sql);
    	if(count($dados))
    	{
	...
	public function buscaCronometro()
	{
		$sql = "SELECT * FROM cronometro_sp WHERE tabelareferencia = '{$this->tabelareferencia}' AND idreferencia = '{$this->idreferencia}' ";
		$dados = $this->queryDados($sql);
		return $dados;
    ...
    {
    	$sql = "SELECT idcronometro FROM cronometro_sp WHERE idreferencia = {$this->getIdreferencia()} AND tabelareferencia = '{$this->getTabelareferencia()}'";
    	$dados = $this->queryDados($sql);
    	if(count($dados))
    	{
    		$this->setIdcronometro($dados[0]['idcronometro']);
    ...
    {
    	$sql = "UPDATE cronometro_sp SET final_tarefa = '' WHERE tabelareferencia = '{$tabela}' AND idreferencia = '{$idincidente}' ";
