...
class PreIncidentes extends Controller
{
	protected $tplDir = 'pre_incidente';
	...
	public function listaResponsaveis()
	{
		...
		$preIncidente = $this->PreIncidentes->fetchAll( $where );
	...
	public function atualizaResponsavel()
	{
		...
		if( $this->PreIncidentes->update( $data , $where ) )
		{
	...
	private function verificaDesignacaoDePreIncidente( $idPreIncidente )
	{
		...
		$busca = $this->PreIncidentes->fetchAll( $where );
		if( count($busca) > 0 )
		{
	...
	private function criaIncidenteESeuAtendimento( Array $dados )
	{
		...
		$preIncidente = $this->PreIncidentes->fetchRow( $where );
		
		if( !$this->verificaPreExistenciaIdProdemgeEmIncidentes( $preIncidente ) )
		{
			if( !$this->Instalacao->getInstalacaoPeloNome( trim( $preIncidente->designacao ) ) )
				return false;
			...
			$resgateInstalacao = $this->Instalacao->getInstalacaoPeloNome( $preIncidente->designacao );
			...
			$this->Incidentes->setidprodemge( $preIncidente->id_prodemge );
