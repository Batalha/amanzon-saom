...
require_once "controller/Incidente.php";
require_once "controller/Usuario.php";

class PreIncidentesNagios extends Controller
{
	protected $tplDir = 'pre_incidente_nagios';
	...
	public function listaResponsaveis()
	{
		...
		$where = " incidentes = 1 ";
		$lista = $this->Usuarios->fetchAll( $where );
	...
	public function view()
	{
		...
		foreach( $preIncidente as $chave => $incidenteItem )
		{
			...
			$item = $this->Usuarios->fetchAll( $where );
	...
	public function edit()
	{
		...
		$where = " incidentes = 1 ";
		$listaUsuarios = $this->Usuarios->fetchAll( $where );
	...
	private function criaIncidenteESeuAtendimento( Array $dados )
	{
		...
		{
			if( !$this->Instalacao->getInstalacaoPeloNome( $preIncidenteNagios->vsat ) )
				return false;
			...
			$resgateInstalacao = $this->Instalacao->getInstalacaoPeloNome( $preIncidenteNagios->vsat );
			if( $resgateInstalacao ){
				$idAssociacaoInstalacaoIncidente = $this->criaAssociacaoInstalacaoIncidente( $idIncidente , $this->Instalacao->getidinstalacoes() );
				
				//cria telefonemas
				$incidenteController = new Incidente();
				$incidenteController->insereTelefonemas( $idAssociacaoInstalacaoIncidente );
			}
			
			$this->Incidentes->setidincidentes( new Integer( $idIncidente ) );
			$this->Usuarios->setidusuarios( new Integer( $dados['responsavel'] ) );
			...
			$this->Incidentes->setidprodemge( $preIncidenteNagios->id_prodemge );
			if( $this->Incidentes->getUltimoIncidentePeloIdProdemge() )
			{
				$texto = $this->Incidentes->getdescricao();
				$texto .= "\n---------------------------------\n";
				...
				$this->Incidentes->setdescricao( $texto );
				if( $this->Incidentes->edit() != 'ok' ) return false;
			}
		}
	...
	{
		$this->AssociacaoInstalacaoIncidente->setidincidentes( $idIncidente );
		$this->AssociacaoInstalacaoIncidente->setidinstalacoes( $designacao );
		return $this->AssociacaoInstalacaoIncidente->create();
	}
	...
	private function criaIncidente( Zend_Db_Table_Row $preIncidente , $prioridade = 'Média' , $saom = 1 )
	{
		...
		if( $id = $this->Incidentes->insert( $data ) )
			return $id;
		else
			return false;
	}
	...
	private function criaAtendimentoParaIncidente( IncidentesBO $incidente , UsuariosBO $responsavel )
	{
		...
		if( $id  = $this->Atendimento->insert( $data ) )
			return $id;
		else
			return false;
	}
	...
	public function listaHistoricoDesignacao()
	{
		...
		$listaIncidentesDeDesignacao = $this->Incidentes->buscaIncidentesFechadosDeDesignacao( $dados['designacao'] );
		if( $listaIncidentesDeDesignacao == false )
			$this->smarty->assign( 'msg' , "Não existem incidentes fechados para essa designação." );
		else
		{
			$listaIncidentesDeDesignacao = $this->Atendimento->iteraListaIncidentesBuscandoAtendimentos( $listaIncidentesDeDesignacao );
			if( $listaIncidentesDeDesignacao == false )
				$this->smarty->assign( 'msg' , "Não existem atendimentos para os incidentes dessa designação." );
			else
			{
				$listaIncidentesDeDesignacao = $this->AssociacaoAtendimentoMotivo->buscaMotivosDeAtendimentos( $listaIncidentesDeDesignacao );
