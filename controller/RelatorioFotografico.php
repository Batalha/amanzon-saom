...
class RelatorioFotografico extends Controller implements RelatorioFotograficoInterface
{
	...
	protected $pastaDeRelatorioFotografico = 'upload/relatorio_fotografico/';
	...
	public function upload()
	{
    	...
    	if( $this->enviaServidor( $this->dadosF , $this->pastaDeRelatorioFotografico ) )
    	{
    		...
    		$arquivo = "<a id='arquivo_novo_relatorio_fotografico' target='_blank' href='upload/relatorio_fotografico/{$this->dadosF['relatorio_fotografico']['name']}'><i class='icon-file'></i><font style='color:#000;'>{$this->dadosF['relatorio_fotografico']['name']}</font></a>";
	...
	public function insereRelatorioFotografico( $dadosF , $id_instalacoes )
	{
		...
		$this->RelatorioFotografico->getRelatorioFotografico( new Integer($id_instalacoes) ); // busca relatorio pre existente

		if( $this->RelatorioFotografico->relatorio != false ) // havendo pre existência
		{
			if( $this->RelatorioFotografico->updateRelatorioFotografico( $dados , new Integer($this->RelatorioFotografico->relatorio['id_relatorio_fotografico']) ) == false )
		...
		else // novo relatorio fotografico
		{
			...
			if( $this->RelatorioFotografico->insertRelatorioFotografico( $dados ) == false )
	...
	{
		if( $this->RelatorioFotografico->aprova( new Integer($this->dadosP['idRelatorio']) ) === true )
	...
	{
		$this->RelatorioFotografico->getRelatorioFotografico( new Integer($this->dadosP['idRelatorio']) );
		if( 
			$this->RelatorioFotografico->desaprova( new Integer($this->dadosP['idRelatorio']) ) === true
			|| $this->RelatorioFotografico->relatorio['status'] == 2 
		)
		{
			$this->RelatorioFotografico->getRelatorioFotografico( new Integer($this->dadosP['idRelatorio']) );
			
			$this->smarty->assign( 'relatorio' , $this->RelatorioFotografico->relatorio );
	...
	{
		if( $this->RelatorioFotografico->justificaDesaprovacao( new Integer($this->dadosP['id_relatorio_fotografico']) , $this->dadosP['comentario'] ) )
	...
	public function apagarRelatorioFotografico()
	{
		...
		$this->RelatorioFotografico->getRelatorioFotografico( new Integer($id_relatorio_fotografico) );
		
		$arquivo = $this->RelatorioFotografico->relatorio['endereco'].$this->RelatorioFotografico->relatorio['nome'];
		unset($arquivo);
		
		if( $this->RelatorioFotografico->deleteRelatorioFotografico( new Integer($this->RelatorioFotografico->relatorio['id_relatorio_fotografico']) ) )
	...
	public function apagarRelatorioFotograficoPeloIdInstalacoes()
	{
		...
		if( $this->RelatorioFotografico->deleteRelatorioFotograficoPeloIdInstalacao( new Integer($id_instalacoes) ) )
