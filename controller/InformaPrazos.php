...
require_once "model/IncidentesModel.php";
...
class PrazosModel extends Controller
{
	...
	private function buscaOS( $dias )
	{
		...
		$where = "
			...
			( 
				( SELECT instalacoes.termo_aceite FROM instalacoes WHERE instalacoes.os_idos = os.idos ) = '' OR 
				( SELECT instalacoes.termo_aceite FROM instalacoes WHERE instalacoes.os_idos = os.idos ) IS NULL 
			) AND
			( -- ----------- tira com data aceite
				( SELECT instalacoes.data_aceite FROM instalacoes WHERE instalacoes.os_idos = os.idos ) = '' OR
				( SELECT instalacoes.data_aceite FROM instalacoes WHERE instalacoes.os_idos = os.idos ) IS NULL
			) AND
			( -- ----------- tira paralisadas
				SELECT COUNT(*)
				FROM pausas
		...
		$this->listaOS = $this->OS->fetchAll( $where , $order );
	}
	...
	private function buscaComissionamentosFinalizados()
	{
		...
		$this->listaInstalacao = $this->Instalacao->fetchAll( $where , $order );
	...
	private function calculaIntervaloDataFinalDataAceite()
	{
		foreach( $this->listaInstalacao as $chave => $instalacao )
		{
			...
			$dataDois = new DateTime( $instalacao['os']['prazoInstal'] );
			...
			$dataQuatro = new DateTime( $instalacao['os']['prazoInstal'] );
	...
	private function buscaOSDasInstalacoes()
	{
		foreach ( $this->listaInstalacao as $chave => $instalacao )
		{
			...
			$os = $this->OS->fetchAll( $where );
			$osArray = $os->toArray();
			$this->listaInstalacao[ $chave ]['os'] = $osArray[0];
		}
	}
...
class InformaPrazoTextoEmail
{
	...
	private $assinaturaSAOM = "
		...
		<img src='http://saom.vodanet-telecom.com/imagens/logo_vodanet.jpg'>
    ";
	...
	private function formaStringInstalacoes( Array $listaInstalacoes )
	{
		foreach ( $listaInstalacoes as $chave => $instalacao )
		{
			...
			$this->msg .= "
				<tr>
					<td style='padding:4px;border:1px solid {$cor};'>{$instalacao['os']['numOS']}</td>
					...
					<td style='padding:4px;border:1px solid {$cor};'>{$instalacao['os']['prazoInstal']}</td>
			...
			";
			$this->msgSemHtml .= "{$instalacao['os'][0]['numOS']} / {$instalacao['nome']} / {$instalacao['data_final_comiss']} / {$instalacao['os']['prazoInstal']} / {$instalacao['intervalo_dt_final_comiss_prazo_instal']} / {$instalacao['data_aceite']} / {$instalacao['os']['prazoInstal']} / {$instalacao['intervalo_data_aceite_prazo_instal']} \r\n";
		}
	}
}
