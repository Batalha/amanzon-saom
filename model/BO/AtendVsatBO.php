...
require_once "model/AtendVsatModel.php";
require_once 'helpers/AdapterZend.php';

include_once 'model/BO/StatusAtendimentoBO.php';
include_once 'model/BO/CronometroBO.php';
include_once 'model/BO/InstalacoesBO.php';
include_once 'model/BO/IncidentesBO.php';
include_once 'model/BO/TipoAtendimentoBO.php';
include_once 'model/BO/UsuariosBO.php';
include_once 'model/BO/StatusAtendimentoBO.php';

include_once 'helpers.class.php';

class AtendVsatBO extends AtendVsatModel
{
	...
	public function __construct( $adapter )
	{
		...
		$this->statusAtendimentos = new StatusAtendimentoBO( $adapter->getAdapterZend() );
		$this->Cronometro = new CronometroBO( $adapter->getAdapterZend() );
		$this->Instalacao = new InstalacoesBO( $adapter->getAdapterZend() );
		$this->Incidente = new IncidentesBO( $adapter->getAdapterZend() );
		$this->TipoAtendimento = new TipoAtendimentoBO( $adapter->getAdapterZend() );
		$this->Usuarios = new UsuariosBO( $adapter->getAdapterZend() );
		$this->Status = new StatusAtendimentoBO( $adapter->getAdapterZend() );
	...
	{
		$where = " incidentes_idincidentes = '{$idIncidente->numero()}' ";
		$order = "";
	...
	private function aplicaIncidenteNosAtendimentos()
	{
		foreach( $this->getlistaAtendimentosArray() as $chave => $atendimento )
		{
			$idIncidente = new Integer( $atendimento['incidentes_idincidentes'] );
...
//		print_b($atendimentos->toArray(),true);
	...
	public function montaObjetoDoForm( $form )
	{
		...
		$this->setincidentes_idincidentes( $form['incidentes_idincidentes'] );
	...
	public function busca_atendimentos_pendentes_para_usuario( Integer $idusuario )
	{
		...
		$select = " DISTINCT(incidentes_idincidentes) ";
		...
		if( count($listaAtendimentosDeUsuario) > 0 )
		{
			...
			{
				if( empty($atendimento['incidentes_idincidentes']) )
					continue;
				
				$where = " 
					incidentes_idincidentes = '{$atendimento['incidentes_idincidentes']}' AND
				...
				if( count($atendimentosDeIncidenteEDeUsuario) > 0 )
					$arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ] = $atendimentosDeIncidenteEDeUsuario->toArray();
				
				//resume textos
				foreach ($arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ] as $chave => $atendimentoDeIncidente)
				{
					$arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['resumo_atendimento'] = substr( $arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['atendimento'] , 0 , 100 ).'...';
					$arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['resumo_resposta_agilis'] = substr( $arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['resposta_agilis'] , 0 , 100 ).'...';
					
					$arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['atendimento'] = nl2br( $arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['atendimento'] );
					$arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['resposta_agilis'] = nl2br( $arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['resposta_agilis'] );
					
					$idStatusAtendimento = new Integer( $arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['status_atend_idstatus_atend'] );
					$arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['status'] = $this->statusAtendimentos->getStatusAtendimento( $idStatusAtendimento );
					
					$arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['data'] = $this->Helpers->data_us_br_com_hora( $arrayAtendimentosPendentesDeUsuario[ $atendimento['incidentes_idincidentes'] ][ $chave ]['data'] );
				}
			}
		}
	...
	public function iteraListaIncidentesBuscandoAtendimentos( Array $listaIncidentes )
	{
		...
		foreach ( $listaIncidentes as $chave => $incidente )
		{
			$sql = "
				SELECT idatend_vsat 
				FROM atend_vsat
				WHERE incidentes_idincidentes = '{$incidente['idincidentes']}'
			";
    ...
    public function getAtendimentosAcompanhamento() {
        ...
        $atendimentos = $this->select()
                             ...
                             ->from(array('av' => 'atend_vsat'), 'av.*')
                             ->joinInner(
                                 array('c' => 'cronometro'),
                                 "(c.tabelareferencia = 'atend_vsat' AND c.idreferencia = av.idatend_vsat)",
                                 'c.*'
                             )
                             ->where('av.incidentes_idincidentes IS NOT NULL AND av.incidentes_idincidentes > 0')
                             ->where("
