...
class DBIncidente extends DBModel 
{
    	...
    	//trata imagens
      	foreach($form as $campo => $val) 
      	{
        	if(in_array($campo,$this->cmpImg))
        	{
				if( ! empty($form[$campo]))
				{
					$move = move_uploaded_file($form[$campo]['tmp_name'], 'imagens/incidentes/'.$form[$campo]['name']);
					if ($move)
					{
                    	$values = "$campo = 'imagens/incidentes/".$form[$campo]['name']."'";
					}
            	}   
        	}
      	}  
    ...
    //PAGINA INICIAL
    	...
    	public function ContaIncidentesAbertos()
		{
			if($_SESSION['login']['empresas_idempresas']==1)//VODANET
     		{
				$sql = "SELECT COUNT(i.idincidentes) total FROM incidentes i WHERE 
				        EXISTS (
     		...
     		{
     			$sql = "SELECT COUNT(i.idincidentes) total FROM incidentes i WHERE 
				        (SELECT o.empresas_idempresas FROM os o WHERE idos = (
				        	SELECT inst.os_idos FROM instalacoes inst WHERE inst.idinstalacoes = i.instalacoes_idinstalacoes)
