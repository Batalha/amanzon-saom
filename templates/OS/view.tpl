...
<div id="acoesOs">
	<table class="acoesTable" style="width: 100%; ">
				...
				{if $login.perfis_idperfis != 3 && $login.perfis_idperfis != 7}
					...
					<div class="divInstOk">
					    ...
					    <input type="button" class="btn btn-primary" value="Ver Dados da VSAT" onClick="javascript:
						    getAjaxForm('Instalacao/view_instalacao','dadosInstal',	{ldelim}param:{$obj.rel.instalacoes.idinstalacoes},ajax:1{rdelim})"
						/>
					                
						<!-- COMISSIONAMENTO -->
						{if $obj.rel.instalacoes.comiss != 1}
							<input type="button" class="btn btn-primary" value="Comissionar" onClick="javascript:
								$.ajax({ldelim} url: 'OS/verificaIplanIpdvb/'+{$obj.idos}, async:false {rdelim}).done(function(response) 
								{ldelim}
						        	if(response == 1)
							            ...
							            {ldelim} 
						            		if(confirm('Está preparado para Comissionamento?\n Obs.: a partir dessa confirmação o tempo de execução da tarefa começará a ser contabilizado.'))
							            		{ldelim}
							            			getAjaxForm('Comissionamento/comiss','dadosInstal',
									    				{ldelim}
						    ...
						    <div class="modal hide hide" id="modal">
							    <form action="OS/edit" method="post" name="edicao_critica_os" id="edicao_critica_os">
									...
									<div class="modal-footer">
										...
										<a href="#" class="btn btn-primary" onclick="javascript: sendPost( OS/edicao_critica','edicao_critica_os')">Salvar Dados</a>
									</div>  	
								</form>
							</div>
						{else}
							<input type="button" class="btn btn-primary" value="Ver Comissionamento" onClick="javascript: guardaReserva({$obj.rel.instalacoes.idinstalacoes},'#idInstalacoes_reserva');
					             getAjaxForm('Comissionamento/comiss_view', 'dadosInstal',
							     {ldelim}
							      	param:{$obj.rel.instalacoes.idinstalacoes},ajax:1
							     {rdelim})" />
						{/if}
						
						<input type="button" id="veros" class="btn btn-primary" value="Ver Dados da OS" onclick="return btmenuOS(this);">
			            ...
			            {if !$pausado}
				            {if $obj.rel.instalacoes.comiss == 1 && $usuario_permissao == 1}
				            	{if $obj.rel.instalacoes.data_aceite == ''}
				            		<input onclick="javascript:getAjaxForm('Instalacao/edit_data_comiss', 'dadosInstal',{ldelim}param:{$obj.rel.instalacoes.idinstalacoes},ajax:1{rdelim})" type="button" class="btn btn-primary" style="float:right;margin-right:10px;" value="Cadastrar Data de Aceite"/>
				            	{else}
				            		<input onclick="javascript:getAjaxForm('Instalacao/edit_data_comiss', 'dadosInstal',{ldelim}param:{$obj.rel.instalacoes.idinstalacoes},ajax:1{rdelim})" type="button" class="btn btn-primary" style="float:right;margin-right:10px;" value="Mudar Data de Aceite"/>
				            	{/if}
				            {/if}
				        {/if}
					</div>
				    {else}
				        <div class="divInstAviso">
				        	...
				        	<input class="btn" type="button" value="Cadastrar dados da VSAT" {if isset($obj.rel.agenda_instal.idagenda_instal)} onClick="javascript:getAjaxForm('Instalacao/create', 'dadosInstal',{ldelim}param:{$obj.idos},ajax:1{rdelim},getDadosInstal,{$obj.idos})" {else} onClick="javascript:simpleMsg('Nenhuma instalação ao campo foi agendada, portanto os dados da instalação não podem ser preenchidos.')"{/if}/>
				        	<input type="button" id="veros" class="btn btn-primary" value=" Ver Dados da OS" onclick="return btmenuOS(this);">
				        </div>            
				    {/if}
				{/if}
		...
		<tr height="40" style=" border-bottom: 1px solid #BABABA;">
			<td width="14%">
				<select name="arquivo" id="arquivo" class="form-control" style="width: 220px; margin: 0;" onchange="return btmenuOS(this);">
				{if $obj.os_status_idos_status != 2}
					{if $login.perfis_idperfis != 3 && $login.perfis_idperfis != 7}
...
<div id="dadosOs" hidden="">
    <div class="areaOS">
    	<div id="linha1">
			...
			<table border="1" class="tableDados">
				<tr height="25">
					<td style="font-weight: bold; width: 15%;	 color: #494949">Nome da Escola :</td><td>{$obj.escola}</td>
				</tr>
			</table>
...
{if $obj.os_status_idos_status != 2}
	{if $login.perfis_idperfis != 3 } <!-- para usuarios que não são "Campo" -->
		...
		<input type="button" class="btn btn-primary" value="Editar dados da OS" onClick="javascript:getAjaxForm('OS/edit',false,{ldelim}param:{$obj.idos},ajax:1{rdelim})" />
		<input type="button" class="btn btn-primary" name="cancelaOS" id="cancelaOS" value="Cancelar OS" onClick="javascript:(confirm('Deseja Cancelar essa OS?'))?getAjaxForm('OS/cancela',false,{ldelim}param:{$obj.idos},ajax:1{rdelim}):'';"/>
		...
		{if $login.perfis_idperfis != 3 }
			{if $obj.rel.instalacoes.data_aceite == ''}
				{if $login.perfis_idperfis == 5 || $login.perfis_idperfis == 4 }
					{if $pausado}
						<input class="btn btn-primary" type="button" value="Desparalisar OS" onClick="javascript:getAjaxForm('OS/caixaDespausa','caixaPausa')" />&nbsp;Pausado em {$dataPausado}
					{else}
					   	<input class="btn btn-primary" type="button" value="Paralisar OS" onClick="javascript:getAjaxForm('OS/caixaPausa','caixaPausa')" />
					{/if}
...
<div hidden="">
			...
			{if ! isset($obj.rel.agenda_instal.idagenda_instal)} <!-- Para ausência de agendamento -->
				{if $pausada != 1}
			   		<div class='divAgendAviso'>de instalação não existe para esta Ordem de Serviço
				   		{if $login.perfis_idperfis == 1 || $login.perfis_idperfis == 3 || $login.perfis_idperfis == 2 || $login.perfis_idperfis == 4 || $login.perfis_idperfis == 5 || $login.perfis_idperfis == 7}, 
				   			<input type="button" class="btn btn-primary" value="Agendar instalação"
				   				onClick="javascript:getAjaxForm('AgendaInstal/create','dadosInstal',{ldelim}param:{$obj.idos},ajax:1{rdelim})" /> 
					   	{/if}
			   		</div>           
				{/if}
			{/if}
			...
			{if isset($obj.rel.agenda_instal.idagenda_instal)} <!-- Para presença de agendamento --> 
				{if $obj.rel.agenda_instal.confirm != 1}
					{if $pausada != 1}
			        	...
			        	<input class="btn btn-primary" type="button" value="Ver agendamento" onClick="javascript:getAjaxForm('AgendaInstal/view','dadosInstal',{ldelim}param:{$obj.rel.agenda_instal.idagenda_instal},ajax:1{rdelim})" /> </div>
			        {/if}           
			    {else}
			    	{if $pausada != 1}
	...
	<!-- 			<strong>Licença Anatel:</strong>&nbsp; -->
				...
				<input id="" name="" type="button" value="Enviar Licença Anatel" class="btn btn-primary" onclick="javascript:
					$.ajax({ldelim}
						url:'Comissionamento/formulario_upload_licenca_anatel',
	...
	<!-- 		<strong>Termo Responsabilidade:</strong>&nbsp; -->
			...
			<span id="btn_apagar_termo_responsabilidade"> <!-- btn para apagar -->
				{if $termo_responsabilidade != '' && $login.perfis_idperfis == 3 && $termo_responsabilidade.status != 1}
					<a title="Apagar" onclick="javascript:
						...
						$.ajax({ldelim}
							url:'TermoResponsabilidade/apagarTermoDeResponsabilidade',
			...
			&nbsp;&nbsp;
			{if $login.perfis_idperfis == 3 || $login.perfis_idperfis == 1 || $login.perfis_idperfis == 2 || $login.perfis_idperfis == 5 || $login.perfis_idperfis == 4 || $login.perfis_idperfis == 7} <!-- formulario de termo de responsabilidade -->
				{if !isset($termo_responsabilidade.status) || $termo_responsabilidade.status != 1}
					<input id="input_termo_responsabilidade" name="input_termo_responsabilidade"
						...
						timeout = new Array(); // apaga timeout's
						$.ajax({ldelim}
							url:'TermoResponsabilidade/uploadForm',
							data:{ldelim}idinstalacao:{$obj.rel.instalacoes.idinstalacoes}{rdelim},
							type:'POST',
							success:function(resposta){ldelim}
								$('#arquivoInstal').html(resposta);
				...
				{if $termo_responsabilidade != ''}
					...
					<input class="btn btn-success" type="button" value="Aprovar" value="Aprovar"
						onclick="javascript:
							timeout = new Array(); // apaga timeout's
							$.ajax({ldelim}
								url:'TermoResponsabilidade/aprovar',
								type:'POST',
					...
					<input class="btn btn-danger" type="button" value="Desaprovar" value="Desaprovar"
						onclick="javascript:
							timeout = new Array();
							$.ajax({ldelim}
								url:'TermoResponsabilidade/desaprovar',
								type:'POST',
								data:{ldelim}idTermo:{$termo_responsabilidade.id_termo_responsabilidade}{rdelim},
								success: function( resposta ){ldelim}
									$('#arquivoInstal').html(resposta);
	...
	<!-- 		<strong>Relatório Fotográfico:</strong>&nbsp; -->
			...
			<span id="btn_apagar_relatorio_fotografico"> <!-- btn para apagar -->
				{if $relatorio_fotografico != '' && $login.perfis_idperfis == 3 && $relatorio_fotografico.status != 1}
					<a title="Apagar" onclick="javascript:
						timeout = new Array(); // apaga timeout's
						$.ajax({ldelim}
							url:'RelatorioFotografico/apagarRelatorioFotografico',
			...
			&nbsp;&nbsp;
			{if $login.perfis_idperfis == 3 || $login.perfis_idperfis == 1 || $login.perfis_idperfis == 2 || $login.perfis_idperfis == 5 || $login.perfis_idperfis == 4 || $login.perfis_idperfis == 7} <!-- formulario de relatorio fotografico -->
				{if !isset($relatorio_fotografico.status) || $relatorio_fotografico.status != 1}
					<input id="input_relatorio_fotografico" name="input_relatorio_fotografico" 
						...
						timeout = new Array(); // apaga timeout's
						$.ajax({ldelim}
							url:'RelatorioFotografico/uploadForm',
							data:{ldelim}idinstalacao:{$obj.rel.instalacoes.idinstalacoes}{rdelim},
							type:'POST',
							success:function(resposta){ldelim}
								$('#arquivoInstal').html(resposta);
				...
				{if $relatorio_fotografico != ''}
					...
					<input class="btn btn-success" type="button" value="Aprovar" value="Aprovar"
						onclick="javascript:
							timeout = new Array(); // apaga timeout's
							$.ajax({ldelim}
								url:'RelatorioFotografico/aprovar',
								type:'POST',
					...
					<input class="btn btn-danger" type="button" value="Desaprovar" value="Desaprovar"
						onclick="javascript:
							timeout = new Array();
							$.ajax({ldelim}
								url:'RelatorioFotografico/desaprovar',
								type:'POST',
								data:{ldelim}idRelatorio:{$relatorio_fotografico.id_relatorio_fotografico}{rdelim},
								success: function( resposta ){ldelim}
									$('#arquivoInstal').html(resposta);
...
<div id="acoesOs" style="margin-top: -20px; margin-bottom: 2px; position: relative;">
