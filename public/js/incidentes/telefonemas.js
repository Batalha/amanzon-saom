...
var objetosTelefonemas = new Array();

function limpaJsTelefonemas()
{
	clearInterval( objetosTelefonemas['intervalo'] );
}

function aplicaTelefonemas(){
	$('#flexmeIncidentes tr').each(function(){
		...
		objetosTelefonemas.push( telefonema );
		
		telefonema = 0;
		
	});
	
	objetosTelefonemas['intervalo'] = setInterval( function(){ 
...
function iniciaContagemCronometroTelefonemas(){
	...
	$.each( objetosTelefonemas , function( index , value ){
			...
			if( value.telefonema1.cronometro.status == 'on' ){
				...
				if( value.telefonema1.cronometro.contagem != undefined ){
					...
					if( value.telefonema1.cronometro.contagem.getTime() < value.telefonema1.prazo.getTime() ){
						...
						objetosTelefonemas[index].telefonema1.cronometro.contagem = new Date();
				...
				if( value.telefonema2.cronometro.contagem != undefined ){	
					if( value.telefonema2.cronometro.contagem.getTime() < value.telefonema2.prazo.getTime() ){
						...
						objetosTelefonemas[index].telefonema2.cronometro.contagem = new Date();
				...
				if( value.telefonema3.cronometro.contagem != undefined ){
					...
					if( value.telefonema3.cronometro.contagem.getTime() < value.telefonema3.prazo.getTime() ){
						...
						objetosTelefonemas[index].telefonema3.cronometro.contagem = new Date();
...
function liberaTelefonemaIncidente( associacao , telefonema ){
	...
	$.ajax({
		url: 'TelefonemasParaIncidentes/liberaTelefonemaIncidente',
		type:'POST',
				...
				if( index != 'intervalo' ){
					...
					if( value.associacao == associacao ){
						...
						if( telefonema == 1 ){
							...
							objetosTelefonemas[index].telefonema1.cronometro.status = 'off';
							$('#telefonema1_'+value.associacao).css('background','url(public/imagens/telefone.gif) 0px 40px');
							objetosTelefonemas[index].telefonema2.cronometro.status = 'on';
							...
							objetosTelefonemas[index].telefonema2.cronometro.status = 'off';
							$('#telefonema2_'+value.associacao).css('background','url(public/imagens/telefone.gif) 0px 40px');
							objetosTelefonemas[index].telefonema3.cronometro.status = 'on';
							...
							objetosTelefonemas[index].telefonema3.cronometro.status = 'off';
