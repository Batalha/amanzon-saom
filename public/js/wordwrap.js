...
function wrap(quem){
    ...
    if(quem.nodeType==3){
		...
		if(quem.nodeValue.replace('n','').replace('t','').trim()==''){
		...
		//alert("texto: " + texto + " rn larg_oficial:" + larg_oficial + " rn larg_total:" + larg_total)
...
function wordWrap(){  
    ...
    for(var i=0; i<elementos.length;i++){
        if(elementos.className.indexOf("word-wrap")>-1){
			if(navigator.appName.indexOf("Internet Explorer")>-1){
				elementos.style.wordWrap = "break-word";
			}else{ //os outros nao suportam o css wordwrap
				wrap(elementos); 
			}
        }
    }
