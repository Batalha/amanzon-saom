...
function desenhaTabelaIncidentes()
{
	limpaJsTelefonemas();
	
	$("#flexmeIncidentes").flexigrid({
    	url: 'Incidente/listeFonte',
        dataType: 'json',
        colModel : [
            {display: 'Nº Incidente', 		name : 'idincidentes', width : 80, sortable : true, align: 'left'},
            {display: 'VSAT', 				name : 'nome_instalacao', width : 115, sortable : true, align: 'left'},
        ...
        resizable: false,
        width: 1280,//1124, //*115 para telefonemas
        height: 795,
        singleSelect: true,
        onSuccess: successFlexigridListIncidentes,
        onError: successFlexigridListIncidentes,
        showToggleBtn: false
	});
...
function doCommandListInstalacoes(com, grid) 
{
	...
	if(com == 'Abrir Instalação Selecionado')
	{
		$('.trSelected', grid).each(function() {
			...
			getAjaxForm('Incidente/view','conteudo',{param:id,ajax:1});
		});
	}
}
...
function successFlexigridListIncidentes()
{
	...
	//alert(campos);
	reconstruindoFlexigrid(campos,'Incidente/listeFonteFiltro','Incidente/view','#flexmeIncidentes');
	aplicaTelefonemas();
