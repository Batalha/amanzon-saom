...
function desenhaTabelaAtendimentossp()
{
	$("#flexmeListAtendimentossp").flexigrid({
        ...
        buttons : [
			{name: 'Carregando...', bclass: 'view', onpress : doCommandListAtendimentossp},
			{separator: true}
        ],
        searchitems : [
        ...
        height: 590,
        singleSelect: true,
        onSuccess: successFlexigridListAtendimentossp,
        onError: successFlexigridListAtendimentossp,
        showToggleBtn: false
	});
}
...
/* DESATIVADO */
function doCommandListAtendimentossp(com, grid)
{
	if(com == 'Abrir Atendimento Selecionado')
	{
		$('.trSelected', grid).each(function() {
			...
			getAjaxForm('AtendVsat_sp/view','conteudo',{param:id,ajax:1})
		});
	}
}


function successFlexigridListAtendimentossp()
{
	...
	controle.reconstuirGridsp(campos,	'AtendVsat_sp/listeFonteFiltro','AtendVsat_sp/view','#flexmeListAtendimentossp');
...
function controleGridAcompanhamento()
{
	...
	this.cronometrosp = function(){
		$('#flexmeListAtendimentossp td[abbr="tempo_passado"] div').each(function(){
			...
			//console.log($('#'+seletor).parent().parent().find('td[abbr="fin"] div').html()!='&nbsp;');
			if(
			...
			if(tempo != 'Finalizado' && tempo != 'Pausado')
			{
				...
				controle = new controleGridAcompanhamento();
				controle.cronoAcompanhamentosp(
	...
	this.cronoAcompanhamentosp = function(hor,min,seg,local){
		var timeCrono;
