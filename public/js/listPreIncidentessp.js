...
function desenhaTabelaPreIncidentessp()
{
	$("#flexmePreIncidentessp").flexigrid({
    	url: 'PreIncidentes_sp/listeFonte',
        dataType: 'json',
        colModel : [
        ...
        buttons : [
			{name: 'Carregando...', bclass: 'view', onpress : doCommandListPreIncidentessp},
			{separator: true}
        ],
        searchitems : [
        ...
        height: 805,
        singleSelect: false,
        onSuccess: successFlexigridListPreIncidentessp,
        onError: successFlexigridListPreIncidentessp,
        showToggleBtn: false
	});
}

/* DESATIVADO */
function doCommandListPreIncidentessp(com, grid)
{
	if(com == 'Abrir Instalação Selecionado')
	{
		$('.trSelected', grid).each(function() {
			...
			//getAjaxForm('PreIncidente_sp/view','conteudo',{param:id,ajax:1})
		});
	}
}

function successFlexigridListPreIncidentessp()
{
	var campos = ["id_pre_incidentes",
	              ...
	              "responsavel",
	              "nome_responsavel"];
	reconstruindoFlexigrid(campos,'PreIncidentes_sp/listeFonteFiltro','PreIncidente_sp/list','#flexmePreIncidentessp');
	aplicaOpcoes();
...
function aplicaOpcoes(){
	$('#flexmePreIncidentessp tr').each(function(){
		var id = $(this).attr('id');
		id = id.substring(id.lastIndexOf('row')+3);
...
function visualizaPreIncidente( idPreIncidente ){
	getAjaxForm('PreIncidentes_sp/view','conteudo',{param:idPreIncidente,ajax:1});
}
...
function abreEscolhaResponsaaveis( idPreIncidente ){
	...
	$.ajax({
		url:'PreIncidentes_sp/listaResponsaveis',
		type:'POST',
