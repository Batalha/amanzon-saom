function desenhaTabelaAcompanhamentosChamadosFullCamposp() {
    $("#flexmeListChamadosFullsp").flexigrid({
        url: 'Acompanhamento_sp/acompanhamentoCampoConteudo',
        dataType: 'json',
        colModel: [
            ...
            {display: 'Solicitação', name: 'solicitacao', width: 115, sortable: true, align: 'left'},
            {display: 'Incidente', name: 'incidente', width: 60, sortable: true, align: 'left'},
            {display: 'Vsat', name: 'vsat', width: 140, sortable: true, align: 'left'},
            {display: 'Perfil', name: 'perfil', width: 100, sortable: true, align: 'left', hide: true},
        ...
        sortorder: "desc",
        usepager: true,
        title: "Lista de Acompanhamentos Campo",
        useRp: true,
        rp: 20,
        ...
        height: 520,
        singleSelect: true,
        onSuccess: successFlexigridListChamadosFullCamposp,
        onError: successFlexigridListChamadosFullCamposp,
        showToggleBtn: false,
        query: ''
    });
}
...
function successFlexigridListChamadosFullCamposp() {
    ...
    var controle = new controleGridCamposp();
    controle.reconstuirGrid(
        campos,
        'Acompanhamento_sp/acompanhamentoCampoConteudoFiltro',
        'OSSP/view',
...
function controleGridCamposp() {
    ...
    this.cronometro = function () {
        $('#flexmeListChamadosFullsp td[abbr="tempo_vencimento"] div').each(function () {
            ...
            //alert(tempo);
            //console.log($('#'+seletor).parent(parent().find('td[abbr="status"] div').html());
            ...
            if ($('#' + seletor).parent().parent().find('td[abbr="perfil"] div').html() == 'noc') {
                var perfil = 'noc';
            ...
            if (tempo != 'Finalizado' && tempo != 'Pausado') {
                ...
                controle = new controleGridCamposp;
                controle.cronoCampo(
    ...
    this.cronoCampo = function (hor, min, seg, local, perfil) {
        var timeCrono;
        StartCrono_corridoCampo(hor, min, seg, local, perfil);
    }
    ...
    this.reload = function () {
        ...
        setTimeout('verificaAualizacaoDisponivelCampo()', 60000);
    }
}
...
function verificaAualizacaoDisponivelCampo() {
...
function atualizaCampoSp() {
    ...
    if ($("#flexmeListChamadosFullsp").html() == null) {
        ...
        setTimeout('refreshCampo()', 1500);
        //$("#flexmeListChamadosFull").flexReload();
    }
}

function refreshCampo() {
    getAjaxForm("Acompanhamento_sp/acompanhamentoCampo");
}

/*
 * hor,min,seg -> tempo corrido
 */
function StartCrono_corridoCampo(hor, min, seg, local, perfil) {
    ...
    setTimeout("StartCrono_corridoCampo(" + hor + "," + min + "," + seg + ",'" + local + "')", 1000);
}

/*
 * hor,min,seg -> tempo que falta
 */
function StartCrono_faltaCampo(hor, min, seg, local) {
    /* interrompe se não tem disponível o campo (para casos de outras páginas) */
    if ($('#' + local).html() == null) {
        return 0;
    }
    ...
    seg--;
    setTimeout("StartCrono_faltaCampo(" + hor + "," + min + "," + seg + ",'" + local + "')", 1000);
}